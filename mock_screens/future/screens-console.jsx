  ...
  function CoachConsole() {
    ...
    const run = async (text, focus) => {
      ...
      if (isThin) {
        ...
        const rec = ENGINE.buildWorkout(member, { focus: f, request: text });
        setRecommendations(rs => [rec, ...rs]);
        asst = { id: 'a' + Date.now(), role: 'assistant', kind: 'workout', rec,
          text: `Here's a ${f}-body session for ${member.name.split(' ')[0]}, built around available equipment and filtered for active injuries. ${rec.validation.corrected.length ? 'The safety validator corrected 1 exercise.' : 'All exercises passed safety validation.'}`,
          trace: rec.trace };
      }
