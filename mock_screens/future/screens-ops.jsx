...
(function () {
  ...
  function EvalScreen() {
    ...
    const M = DB.metrics;
    const runAll = async () => { setRunning(true); setResults(null); await new Promise(r => setTimeout(r, 1400)); setResults(DB.scenarios.map(s => ({ ...s, actual: s.status }))); setRunning(false); toast({ title: 'Ran ' + DB.scenarios.length + ' critical-path tests', detail: '4 pass · 1 warn', sev: 'success' }); };
        ...
        React.createElement('div', { className: 'col gap10' },
          (results || DB.scenarios).map(s => React.createElement('div', { key: s.id, className: 'card', style: { padding: 14, background: 'var(--bg-2)' } },
            ...
            React.createElement('div', { className: 'row gap8' },
              ...
              React.createElement(Btn, { size: 'sm', variant: 'ghost', icon: 'play', onClick: () => toast({ title: 'Running scenario: ' + s.name, sev: 'info' }) }, 'Run'),
  ...
  function SystemTrace() {
    ...
    const trace = useMemo(() => ENGINE.buildTrace(member, {}), [member]);
    const requests = [
      { id: 'req_1', type: 'recommend', label: 'Build lower-body session', status: 'ok', latency: 4.8 },
      { id: 'req_2', type: 'explain', label: 'Why skipped barbell squat', status: 'ok', latency: 1.9 },
      { id: 'req_3', type: 'retrieve', label: 'Vector index stale → graph-only fallback', status: 'warn', latency: 2.1 },
      { id: 'req_4', type: 'recommend', label: 'Invalid exercise ID caught', status: 'corrected', latency: 5.1 },
    ];
    return React.createElement('div', { className: 'screen' }, React.createElement('div', { className: 'screen-pad wide' },
      React.createElement(PageHead, { title: 'System Trace', sub: 'Request → retrieval → LLM/tool calls → validation → safety, with latency budgets' },
        React.createElement(Btn, { icon: 'copy', onClick: () => copy(trace, 'Trace copied') }, 'Copy trace'),
        React.createElement(Btn, { icon: 'export', onClick: () => toast({ title: 'Demo transcript exported', sev: 'success' }) }, 'Export demo transcript')),
      React.createElement('div', { className: 'col gap10' }, requests.map((req, i) => React.createElement('div', { key: req.id, className: 'card' },
        React.createElement('button', { className: 'panel-head', style: { width: '100%', background: 'none', border: 'none', cursor: 'pointer' }, onClick: () => setExpanded(expanded === i ? -1 : i) },
          ...
          React.createElement('span', { className: `sb ${req.status === 'ok' ? 'safe' : req.status === 'warn' ? 'caution' : 'info'}` }, req.status),
        ...
        expanded === i && React.createElement('div', { className: 'card-pad', style: { borderTop: '1px solid var(--line-soft)' } },
          React.createElement('div', { className: 'col gap6 mb16' }, trace.stages.map((s, j) => {
            const budget = DB.latencyStages[Math.min(j, 3)]?.budget || 2;
            ...
            return React.createElement('div', { key: j, className: 'row gap12 center' },
              ...
              React.createElement('div', { className: 'grow', style: { height: 16, borderRadius: 5, background: 'var(--bg-3)', position: 'relative', overflow: 'hidden' } },
                React.createElement('div', { style: { height: '100%', width: Math.min(100, s.latency / budget * 100) + '%', background: over ? 'var(--danger)' : 'var(--grad)' } }),
                React.createElement('span', { style: { position: 'absolute', right: 8, top: 0, fontSize: 10, lineHeight: '16px', color: 'var(--ink-1)' }, className: 'mono' }, s.latency.toFixed(1) + 's / ' + budget + 's')),
              s.tokens > 0 && React.createElement('span', { className: 'fs11 faint mono', style: { width: 60, textAlign: 'right' } }, s.tokens + ' tok'));
          })),
          req.status === 'corrected' && React.createElement('div', { className: 'card mb12', style: { padding: 10, background: 'var(--info-bg)' } }, React.createElement('span', { className: 'fs12', style: { color: 'var(--info)' } }, 'Validation caught unknown exercise ID ', React.createElement('span', { className: 'mono' }, 'ex_999'), ' → corrected to ', React.createElement('span', { className: 'mono' }, 'ex_008'))),
          req.status === 'warn' && React.createElement('div', { className: 'card mb12', style: { padding: 10, background: 'var(--caution-bg)' } }, React.createElement('span', { className: 'fs12', style: { color: 'var(--caution)' } }, 'Vector index stale — fell back to graph-only retrieval.')),
