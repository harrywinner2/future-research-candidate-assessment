  ...
  function buildGraph(mbr) {
    ...
    mbr.injuries.forEach(inj => {
      ...
      if (inj.status === 'active' || inj.status === 'improving') {
        exercises.filter(ex => ex.joints_loaded.includes(inj.joint)).slice(0, 7).forEach(ex => {
    ...
    const safeEx = exercises.filter(ex => evalExerciseForMember(ex, mbr).state === 'safe').slice(0, 6);
    safeEx.forEach(ex => {
      add({ id: ex.id, type: 'Exercise', label: ex.name, ex });
